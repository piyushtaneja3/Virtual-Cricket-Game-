...
void Game::tossChoice(Team tossWinnerTeam) {
	...
	switch (tossInput) {
		...
		case 2:
			...
			if (tossWinnerTeam.name.compare("Team-A") == 0) {r
				bowlingTeam = &teamA;
				battingTeam = &teamB;
			} else {	
				bowlingTeam = &teamB;
...
bool Game::validateInningsScore(){
	if(isFirstInnings){
		...
		cout << "\t\t ||| SECOND INNINGS ENDS ||| " << endl << endl;
								...
								decideWinner(teamA.totalRunsScored,teamB.totalRunsScored);
								return false;
	}
	return true;
}
void Game::showGameScoreCard(){
	...
	cout<<battingTeam->name
			...
			<< battingTeam->totalRunsScored<<"- "<<battingTeam->wicketsLost<<" ( "<<bowlingTeam->totalBallsBowled<<" ) "<<" | "<<batsman->name <<batsman->runsScored<<" ( "<<batsman->ballsPlayed<<")"<<"\t\t"<<bowler->name<<bowler->ballsBowled<<" - "<<bowler->  runsGiven <<" - "<< bowler->wicketsTaken <<endl<<endl;
	cout<<"-----------------------------"<<endl<<endl;
}
...
void Game::decideWinner(int teamAscore,int teamBscore){
	if(teamAscore>teamBscore){
		cout<<"teamA is the winner "<<endl;
	}
	else if (teamBscore>teamAscore){
		cout<<"teamB is the winner "<<endl;
	}
	else{
